...
//   const [form, setForm] = useState({ title: "", location: "", date: "", seats: 0 });
...
//       if (editingId) {
//         await axios.put(`${API}/api/events/${editingId}`, formData, {
...
//   // ✅ Edit Event
//   const handleEdit = (ev) => {
//     setForm({ title: ev.title, location: ev.location, date: ev.date, seats: ev.seats });
//     setEditingId(ev.id);
//   };
...
//       <motion.div className="container mx-auto px-4 py-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
//         <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800 text-center">Admin Dashboard</h1>

//         {/* Event Form */}
//         <motion.div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg mb-6" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
//           <h2 className="text-lg sm:text-xl font-semibold mb-3 text-gray-700">{editingId ? "✏️ Edit Event" : "➕ Add New Event"}</h2>

//           <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
//             <input name="title" value={form.title} onChange={handleChange} placeholder="Event Title" className="border rounded-xl p-2 focus:ring focus:ring-blue-300 outline-none" />
//             <input name="location" value={form.location} onChange={handleChange} placeholder="Location" className="border rounded-xl p-2 focus:ring focus:ring-blue-300 outline-none" />
//             <input name="date" type="date" value={form.date} onChange={handleChange} className="border rounded-xl p-2 focus:ring focus:ring-blue-300 outline-none" />
//             <input name="seats" type="number" value={form.seats} onChange={handleChange} placeholder="Available Seats" className="border rounded-xl p-2 focus:ring focus:ring-blue-300 outline-none" />
//             <input type="file" accept="image/*" onChange={handleImageChange} className="border rounded-xl p-2 focus:ring focus:ring-blue-300 outline-none sm:col-span-2" />
//           </div>

//           <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleAddOrUpdate} className="mt-4 bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 w-full sm:w-auto">
...
//           ) : (
//             <motion.table layout className="min-w-full border-collapse border border-gray-300 shadow rounded-xl">
...
//               <tbody>
//                 {events.map((ev) => (
//                   <motion.tr key={ev.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="hover:bg-gray-50 transition text-xs sm:text-sm">
...
//                       {ev.image && <img src={ev.image} alt={ev.title} className="w-16 h-12 sm:w-20 sm:h-16 object-cover rounded" />}
//                     </td>
//                     <td className="border p-2 flex flex-col sm:flex-row gap-2">
//                       <button onClick={() => handleEdit(ev)} className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</button>
//                       <button onClick={() => handleDelete(ev.id)} className="bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700">Delete</button>
//                     </td>
//                   </motion.tr>
//                 ))}
//               </tbody>
...
//               <tbody>
//                 {bookings.map((b) => (
//                   <motion.tr key={b.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="hover:bg-gray-50 transition text-xs sm:text-sm">
...
//                     <td className="border p-2">{b.eventDate}</td>
//                   </motion.tr>
//                 ))}
//               </tbody>
//             </table>
...
//     try {
//       const res = await axios.get(`${API}/api/events`);
//       console.log("Events fetched:", res.data); // check Cloudinary URLs
//       setEvents(res.data);
//     } catch (err) {
...
//                       {ev.image && (
//                         <img
//                           src={ev.image} // ✅ Cloudinary URL directly
//                           alt={ev.title}
...
//   const handleAddOrUpdate = async () => {
//     try {
//       if (!form.title || !form.location || !form.date || !form.seats) {
...
//   // ✅ Edit event
//   const handleEdit = (ev) => {
//     setForm({ title: ev.title, location: ev.location, date: ev.date.split("T")[0], seats: ev.seats });
//     setEditingId(ev.id);
//   };
...
//           <h2 className="text-xl font-semibold mb-3">{editingId ? "Edit Event" : "Add New Event"}</h2>
//           <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
//             <input
//               name="title"
//               value={form.title}
...
//                     <td className="border p-2">{ev.title}</td>
//                     <td className="border p-2">{ev.location}</td>
//                     <td className="border p-2">{new Date(ev.date).toLocaleDateString()}</td>
//                     <td className="border p-2">{ev.seats}</td>
//                     <td className="border p-2">
//                       {ev.image && <img src={ev.image} alt={ev.title} className="w-16 h-12 object-cover rounded" />}
//                     </td>
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id} className="hover:bg-gray-50">
                    <td className="border p-2">{b.booking_id}</td>
                    <td className="border p-2">{b.event_title}</td>
                    <td className="border p-2">{b.name}</td>
                    <td className="border p-2">{b.email}</td>
                    <td className="border p-2">{b.seats}</td>
                  </tr>
